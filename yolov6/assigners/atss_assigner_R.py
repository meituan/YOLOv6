...
class ATSSAssigner(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, anc_bboxes, n_level_bboxes, gt_labels, gt_bboxes, gt_angles, mask_gt, pd_bboxes):
        ...
        if self.n_max_boxes == 0:
            ...
            return (
                ...
                torch.zeros([self.bs, self.n_anchors, self.angle_max]).to(device),
        ...
        target_labels, target_bboxes, target_angles, target_scores = self.get_targets(
            gt_labels, gt_bboxes, gt_angles, target_gt_idx, fg_mask
        )
        ...
        return target_labels.long(), target_bboxes, target_angles, target_scores, fg_mask.bool()
    ...
    def get_targets(self, gt_labels, gt_bboxes, gt_angles, target_gt_idx, fg_mask):
        ...
        # assigned angles boxes
        target_angles = gt_angles.reshape([-1, 1])[target_gt_idx.flatten()]
        target_angles = target_angles.reshape([self.bs, self.n_anchors, 1])
